...
def prob_to_transit(rstar, logg, per, rplanet):
    ...
    r_earth = 6.371e8 # cm
    au2cm = 1.49598e13 # 1 AU = 1.49598e13 cm
    prob = np.zeros((rplanet.size, per.size))
    for i in range(rplanet.size):
	for j in range(per.size):
    	    prob[i,j] = (rstar*r_sun + rplanet[i]*r_earth) / (semia[j]*au2cm)
    # prob to transit e > 0
    #prob = prob / (1.0-ecc**2)
    prob *= 1.08  # from Kipping 2013 eccentricity paper
...
def kepler_single_comp_dr25(data):
    ...
    # THIS function has been modified by RC
    probtransit_2d = prob_to_transit(data.rstar, data.logg,
                                     data.period_want, data.rp_want)
...
if __name__ == "__main__":
    ...
    doit.id = 8311864#3429335
    doit.rstar = 0.56735763#0.798
    doit.logg = 4.72392535#4.578
    doit.teff = 3652.54012#5554.0
    doit.dataspan = 1458.93
    doit.dutycycle = 0.874
    doit.limbcoeffs = np.array([0.5464,0.3227,0.0219,-0.0633])#np.array([0.4869,0.05340,0.5129,-0.3007])
